...
const ProductCard = ({ product }: ProductCardProps) => {
  ...
  return <div onMouseEnter={() => setProductHover(product.id, true)} onMouseLeave={() => setProductHover(product.id, false)} className="relative w-[20rem] m-[1rem] border-[#ECECEC] ml-[3rem] p-4">
      ...
      {hover && (
        <div className="absolute inset-0 flex items-center justify-between px-4">
          <button onClick={handlePrev} className="carousel-button text-white ">
            <FaChevronLeft className="bg-gray-300 rounded-full cursor-pointer" />
          </button>

          <button onClick={handleNext} className="carousel-button text-white ">
            <FaChevronRight className="bg-gray-300 rounded-full cursor-pointer" />
          </button>
        </div>                            
      )}
    </div>
